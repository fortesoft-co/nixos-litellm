        ...
        options = {
          api_base = mkOption {
            ...
            description = ''
              ...
              or "https://api.openai.com/v1").  Set to null for providers that
              derive their endpoint from credentials alone (e.g. vertex_ai,
              bedrock).  When null, no api_base is emitted in the generated
              model_list entry.

              Required when autoDiscover is true (the discovery script needs a
              URL to query).
            '';
          };
          ...
          api_key = mkOption {
            ...
            description = ''
              ...
              a secret declared in cfg.litellm.secrets.  When null, no api_key
              is emitted in the generated model_list entry.

              For the "ollama" provider, a null api_key is automatically
              filled in as "ollama" at config-generation time — local Ollama
              ignores the value but LiteLLM historically requires it to be
              present.  Set this explicitly to override that behaviour.
            '';
          };
          ...
          litellm_params = mkOption {
            ...
            default = { };
            description = ''
              Extra fields merged into the litellm_params dict of every
              model_list entry generated by this endpoint (both explicit
              models and the wildcard entry, as well as auto-discovered
              models).  Use this for provider-specific options that don't fit
              the api_base/api_key pattern.

              These are merged on top of model/api_base/api_key, so keys
              here take precedence and can be used to override either of
              those for a specific provider's expected format.

              Example (vertex_ai — no api_base/api_key, GCP credentials
              instead):

                  litellm_params = {
                    vertex_project = "os.environ/VERTEX_PROJECT";
                    vertex_location = "us-central1";
                    vertex_credentials = "os.environ/VERTEX_CREDS";
                  };
            '';
          };
